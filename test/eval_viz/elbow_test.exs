defmodule EvalViz.ElbowTest do
  ...
  describe "elbow/3" do
    test "returns a VegaLite spec" do
      assert %VegaLite{} = EvalViz.elbow(ks(), inertias())
    end

    test "plots one point per k" do
      assert values(EvalViz.elbow(ks(), inertias())) |> Enum.map(& &1["k"]) == ks()
    end

    test "finds the corner" do
      assert rule(EvalViz.elbow(ks(), inertias()))["data"]["values"] == [%{"k" => 3}]
      assert spec(EvalViz.elbow(ks(), inertias()))["title"]["subtitle"] == "elbow at k = 3"
    end
    ...
    test "finds the same corner when the score is rescaled" do
      ...
      assert rule(EvalViz.elbow(ks(), scaled))["data"]["values"] == [%{"k" => 3}]
    end

    test "mark_elbow: false leaves the rule and subtitle out" do
      plot = EvalViz.elbow(ks(), inertias(), mark_elbow: false)
    ...
    test "marks nothing when there are too few points to have a corner" do
      plot = EvalViz.elbow([2, 3], [500.0, 120.0])
    ...
    test "marks nothing when every score is the same" do
      plot = EvalViz.elbow(ks(), [10.0, 10.0, 10.0, 10.0, 10.0])
      assert length(spec(plot)["layer"]) == 1
    end
    ...
    test "marks nothing when the scores fall in a straight line" do
      plot = EvalViz.elbow(ks(), [500.0, 400.0, 300.0, 200.0, 100.0])
    ...
    test "accepts tensors" do
      from_tensors = values(EvalViz.elbow(Nx.tensor(ks()), Nx.tensor(inertias(), type: :f64)))
    ...
    test "lets the score axis start where the data does" do
      layer = spec(EvalViz.elbow(ks(), inertias()))["layer"] |> List.last()
      assert layer["encoding"]["y"]["scale"]["zero"] == false
    end

    test "renames the axes" do
      plot = EvalViz.elbow(ks(), inertias(), metric_title: "Distortion", k_title: "Groups")
