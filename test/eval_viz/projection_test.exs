defmodule EvalViz.ProjectionTest do
  ...
  describe "projection/3" do
    test "returns a VegaLite spec" do
      assert %VegaLite{} = EvalViz.projection(embedding())
    end

    test "draws one point per row" do
      assert length(values(EvalViz.projection(embedding()))) == 4
    end

    test "takes the coordinates from the chosen columns" do
      rows = values(EvalViz.projection(embedding()))
    ...
    test "reads a struct that carries the embedding, as MDS does" do
      from_struct = values(EvalViz.projection(%{embedding: embedding()}))
      assert from_struct == values(EvalViz.projection(embedding()))
    end

    test "picks out the components it is given" do
      wide = Nx.tensor([[0.0, 9.0, 1.0], [2.0, 9.0, 3.0]])
      plot = EvalViz.projection(wide, components: [0, 2])
    ...
    test "leaves colour alone when there are no labels" do
      refute encoding(EvalViz.projection(embedding()))["color"]
    end

    test "colours by label" do
      plot = EvalViz.projection(embedding(), labels())
    ...
    test "names the labels when asked" do
      plot = EvalViz.projection(embedding(), labels(), label_names: ["left", "right"])
    ...
    test "gives both axes one range" do
      plot = EvalViz.projection(embedding())
    ...
    test "lets the axes scale apart when told to" do
      plot = EvalViz.projection(embedding(), equal_axes: false)
  ...
  describe "side by side" do
    ...
    test "keeps its own title above the panels" do
      plot = EvalViz.projection(embedding(), [{"A", labels()}, {"B", labels()}], title: "Both")
      assert spec(plot)["title"] == "Both"
    end

    test "uses the panel title when only one is given" do
      plot = EvalViz.projection(embedding(), [{"Only", labels()}])
    ...
    test "rejects a panel that is not a {title, labels} pair" do
      assert_raise ArgumentError, ~r/expected \{title, labels\}/, fn ->
        EvalViz.projection(embedding(), [labels()])
      end
    end
  end
end
