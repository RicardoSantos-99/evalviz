defmodule EvalViz.ScreeTest do
  ...
  describe "scree/2" do
    test "returns a VegaLite spec" do
      assert %VegaLite{} = EvalViz.scree(ratios())
    end

    test "numbers components from one" do
      assert values(EvalViz.scree(ratios())) |> Enum.map(& &1["component"]) == [1, 2, 3, 4]
    end

    test "keeps each component's own share" do
      assert values(EvalViz.scree(ratios()))
             |> Enum.map(& &1["ratio"])
    ...
    test "accumulates the running total up to one" do
      cumulative = values(EvalViz.scree(ratios())) |> Enum.map(& &1["cumulative"])
    ...
    test "accepts a PCA model as well as a tensor" do
      ...
      assert values(EvalViz.scree(model)) == values(EvalViz.scree(ratios()))
    end

    test "layers the running total over the bars" do
      [bars, line] = spec(EvalViz.scree(ratios()))["layer"]
    ...
    test "cumulative: false leaves only the bars" do
      assert length(spec(EvalViz.scree(ratios(), cumulative: false))["layer"]) == 1
    end

    test "labels the variance axis as a percentage" do
      [bars, _line] = spec(EvalViz.scree(ratios()))["layer"]
      assert bars["encoding"]["y"]["axis"]["format"] == ".0%"
    end

    test "handles a single component" do
      plot = EvalViz.scree(Nx.tensor([1.0]))
