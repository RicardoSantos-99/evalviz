defmodule EvalViz.ModelComparisonTest do
  ...
  describe "predicted_vs_actual/3 comparing models" do
    test "carries every model's points" do
      assert length(values(EvalViz.predicted_vs_actual(two_models()))) == 8
      assert series(EvalViz.predicted_vs_actual(two_models())) == ["Linear", "Ridge"]
    end

    test "colours by model" do
      encoding = scatter(EvalViz.predicted_vs_actual(two_models()))["encoding"]
    ...
    test "fits one range to every model" do
      only_linear = EvalViz.predicted_vs_actual(y_true(), linear())
      both = EvalViz.predicted_vs_actual(two_models())
    ...
    test "puts the diagonal across that same range" do
      plot = EvalViz.predicted_vs_actual(two_models())
    ...
    test "leaves a single model exactly as it was" do
      plot = EvalViz.predicted_vs_actual(y_true(), linear())
  ...
  describe "residuals/3 comparing models" do
    test "carries every model's residuals" do
      assert length(values(EvalViz.residuals(two_models()))) == 8
      assert series(EvalViz.residuals(two_models())) == ["Linear", "Ridge"]
    end

    test "still takes the residual as prediction minus truth" do
      linear_residuals =
        EvalViz.residuals(two_models())
        |> values()
    ...
    test "colours by model" do
      assert scatter(EvalViz.residuals(two_models()))["encoding"]["color"]["field"] == "series"
    end

    test "keeps one zero line for all of them" do
      [zero, _scatter] = spec(EvalViz.residuals(two_models()))["layer"]
      assert zero["data"]["values"] == [%{"zero" => 0}]
    end

    test "leaves a single model exactly as it was" do
      plot = EvalViz.residuals(y_true(), linear())
  ...
  describe "learning_curve/4 comparing models" do
    ...
    test "tags every row with its model" do
      plot = EvalViz.learning_curve(sizes(), curves())
    ...
    test "moves colour onto the model and the split onto the dash" do
      encoding = line(EvalViz.learning_curve(sizes(), curves()))["encoding"]
    ...
    test "gives the dash legend a swatch that shows the pattern" do
      legend = line(EvalViz.learning_curve(sizes(), curves()))["encoding"]["strokeDash"]["legend"]
    ...
    test "splits the bands on detail, which an area can carry" do
      [band, _line] = spec(EvalViz.learning_curve(sizes(), curves()))["layer"]
    ...
    test "spread: false drops the bands, which is worth it with several models" do
      plot = EvalViz.learning_curve(sizes(), curves(), spread: false)
    ...
    test "rejects scores that do not line up, naming the model's argument" do
      assert_raise ArgumentError, ~r/train_scores to have one entry per training size/, fn ->
        EvalViz.learning_curve(sizes(), [{"Linear", [0.9], [0.7, 0.8, 0.85]}])
      end
    end
  end
end
