defmodule Verdict.LoadingsTest do
  ...
  describe "loadings/2" do
    test "returns a VegaLite spec" do
      assert %VegaLite{} = Verdict.loadings(components())
    end

    test "draws one cell per component and feature" do
      assert length(values(Verdict.loadings(components()))) == 6
    end

    test "lays components out as rows and features as columns" do
      plot = Verdict.loadings(components())
    ...
    test "centres the colour scale on zero" do
      [min, max] = heatmap(Verdict.loadings(components()))["encoding"]["color"]["scale"]["domain"]
    ...
    test "uses a diverging scheme, not a sequential one" do
      assert heatmap(Verdict.loadings(components()))["encoding"]["color"]["scale"]["scheme"] ==
    ...
    test "writes the value in each cell" do
      assert Verdict.loadings(components()) |> values() |> Enum.map(& &1["label"]) |> hd() ==
               "0.6"

      assert length(spec(Verdict.loadings(components()))["layer"]) == 2
    end

    test "values: false leaves the numbers out" do
      assert length(spec(Verdict.loadings(components(), values: false))["layer"]) == 1
    end
