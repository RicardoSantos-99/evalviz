defmodule Verdict.ConfusionMatrixTest do
  ...
  describe "confusion_matrix/3" do
    test "returns a VegaLite spec" do
      assert %VegaLite{} = Verdict.confusion_matrix(y_true(), y_pred(), num_classes: 3)
    end

    test "encodes one datum per cell, with counts matching the matrix" do
      spec = Verdict.confusion_matrix(y_true(), y_pred(), num_classes: 3)
    ...
    test "normalize: :true_class makes each row sum to 1" do
      spec = Verdict.confusion_matrix(y_true(), y_pred(), num_classes: 3, normalize: :true_class)
    ...
    test "normalize: :all makes the whole matrix sum to 1" do
      spec = Verdict.confusion_matrix(y_true(), y_pred(), num_classes: 3, normalize: :all)
    ...
    test "rejects class_names that do not cover every class" do
      assert_raise ArgumentError, ~r/one entry per class/, fn ->
        Verdict.confusion_matrix(y_true(), y_pred(), num_classes: 3, class_names: ["cat", "dog"])
      end
    end

    test "rejects inputs of different lengths" do
      assert_raise ArgumentError, ~r/same length/, fn ->
        Verdict.confusion_matrix(y_true(), Nx.tensor([0, 1]), num_classes: 3)
      end
    end

    test "rejects inputs that are not rank-1" do
      assert_raise ArgumentError, ~r/rank-1/, fn ->
        Verdict.confusion_matrix(Nx.tensor([[0, 1], [1, 0]]), y_pred(), num_classes: 3)
      end
    end
