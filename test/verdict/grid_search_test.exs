defmodule Verdict.GridSearchTest do
  ...
  describe "grid_search/2" do
    test "returns a VegaLite spec" do
      assert %VegaLite{} = Verdict.grid_search(results())
    end

    test "draws a cell per combination" do
      assert length(values(Verdict.grid_search(results()))) == 4
    end

    test "infers the axes from the hyperparameters that vary" do
      plot = Verdict.grid_search(results())
    ...
    test "takes the axes when given them" do
      plot = Verdict.grid_search(results(), x: :iterations, y: :alpha)
    ...
    test "ignores a hyperparameter that never changes" do
      ...
      assert %VegaLite{} = Verdict.grid_search(fixed)
    end

    test "picks the metric out of each score" do
      plot = Verdict.grid_search(results(), metric: 1, best: :min)
    ...
    test "rejects a metric the scores do not have" do
      assert_raise ArgumentError, ~r/:metric to index the 2 scores/, fn ->
        Verdict.grid_search(results(), metric: 5)
      end
    end
    ...
    test "reverses the ramp when smaller is better" do
      refute heatmap(Verdict.grid_search(results()))["encoding"]["color"]["scale"]["reverse"]

      assert heatmap(Verdict.grid_search(results(), best: :min))["encoding"]["color"]["scale"][
               "reverse"
             ]
    end
    ...
    test "carries a shade that runs from worst to best either way" do
      up = Verdict.grid_search(results())
      down = Verdict.grid_search(results(), best: :min)
    ...
    test "best: :none marks nothing" do
      plot = Verdict.grid_search(results(), best: :none)
    ...
    test "values: false leaves the numbers out" do
      assert length(spec(Verdict.grid_search(results(), values: false))["layer"]) == 2
    end
    ...
    test "collapses extra parameters and says so" do
      ...
      plot = Verdict.grid_search(wide, x: :alpha, y: :iterations)
