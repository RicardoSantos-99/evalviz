defmodule Verdict.ScoreDistributionTest do
  ...
  describe "score_distribution/3" do
    test "returns a VegaLite spec" do
      assert %VegaLite{} = Verdict.score_distribution(y_true(), scores())
    end

    test "draws a series per true class" do
      assert Verdict.score_distribution(y_true(), scores(), bins: 2)
             |> values()
             |> Enum.map(& &1["class"])
             |> Enum.uniq() == ["0", "1"]
    end
    ...
    test "makes each class sum to one by default" do
      plot = Verdict.score_distribution(y_true(), scores(), bins: 2)
    ...
    test "normalize: :none shows the raw counts" do
      plot = Verdict.score_distribution(y_true(), scores(), bins: 2, normalize: :none)
    ...
    test "labels the y axis for the normalization in force" do
      shared = Verdict.score_distribution(y_true(), scores(), bins: 2)
      raw = Verdict.score_distribution(y_true(), scores(), bins: 2, normalize: :none)
    ...
    test "gives the bars an extent on both axes" do
      encoding = bars(Verdict.score_distribution(y_true(), scores()))["encoding"]
    ...
    test "shares one set of bin edges across the classes" do
      plot = Verdict.score_distribution(y_true(), scores(), bins: 4)
    ...
    test "overlays the classes instead of stacking them" do
      rows = values(Verdict.score_distribution(y_true(), scores()))
      assert rows |> Enum.map(& &1["zero"]) |> Enum.uniq() == [0]
    end

    test "names the classes when asked" do
      plot = Verdict.score_distribution(y_true(), scores(), class_names: ["no", "yes"])
    ...
    test "marks a threshold when given one" do
      plot = Verdict.score_distribution(y_true(), scores(), threshold: 0.55)
    ...
    test "draws no rule without a threshold" do
      assert length(spec(Verdict.score_distribution(y_true(), scores()))["layer"]) == 1
    end

    test "rejects inputs of different lengths" do
      assert_raise ArgumentError, ~r/same length/, fn ->
        Verdict.score_distribution(y_true(), Nx.tensor([0.1, 0.2]))
      end
    end
  end
end
