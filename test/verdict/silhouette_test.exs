defmodule Verdict.SilhouetteTest do
  ...
  describe "silhouette/3" do
    test "returns a VegaLite spec" do
      assert %VegaLite{} = Verdict.silhouette(x(), labels(), num_clusters: 2)
    end

    test "draws one bar per point, carrying the score Scholar computes" do
      plot = Verdict.silhouette(x(), labels(), num_clusters: 2)
    ...
    test "gives each bar an explicit extent on both axes" do
      ...
      layer = bars_layer(Verdict.silhouette(x(), labels(), num_clusters: 2))
    ...
    test "average_line: false drops the reference" do
      plot = Verdict.silhouette(x(), labels(), num_clusters: 2, average_line: false)
    ...
    test "keeps room for negative scores instead of clipping at zero" do
      ...
      layer = bars_layer(Verdict.silhouette(x(), bad_labels, num_clusters: 2))
    ...
    test "names clusters when asked" do
      plot =
        Verdict.silhouette(x(), labels(), num_clusters: 2, cluster_names: ["near", "far"])
    ...
    test "rejects cluster_names that do not cover every cluster" do
      assert_raise ArgumentError, ~r/one entry per cluster/, fn ->
        Verdict.silhouette(x(), labels(), num_clusters: 2, cluster_names: ["only one"])
      end
    end

    test "requires num_clusters" do
      assert_raise NimbleOptions.ValidationError, fn ->
        Verdict.silhouette(x(), labels())
      end
    end
  end
end
