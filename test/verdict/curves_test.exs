defmodule Verdict.CurvesTest do
  ...
  describe "roc_curve/3" do
    test "returns a VegaLite spec" do
      assert %VegaLite{} = Verdict.roc_curve(y_true(), scores())
    end

    test "plots the rates scikit-learn computes" do
      points = values(Verdict.roc_curve(y_true(), scores()))
    ...
    test "chance_line: false drops the reference" do
      layers = spec(Verdict.roc_curve(y_true(), scores(), chance_line: false))["layer"]
      assert length(layers) == 1
    end

    test "a single curve carries no colour legend" do
      refute curve_layer(Verdict.roc_curve(y_true(), scores()))["encoding"]["color"]
    end

    test "a single curve shows its AUC as a subtitle, since it has no legend" do
      assert spec(Verdict.roc_curve(y_true(), scores()))["title"] == %{
               "text" => "",
               "subtitle" => "AUC = 0.75"
             }

      assert spec(Verdict.roc_curve(y_true(), scores(), title: "Held out"))["title"] == %{
               "text" => "Held out",
               "subtitle" => "AUC = 0.75"
             }
    end
    ...
    test "shows markers for few thresholds and hides them for many" do
      assert curve_layer(Verdict.roc_curve(y_true(), scores()))["mark"]["point"] == true

      many_true = Nx.tensor(Enum.map(1..200, &rem(&1, 2)))
      many_scores = Nx.tensor(Enum.map(1..200, &(&1 / 200)))

      assert curve_layer(Verdict.roc_curve(many_true, many_scores))["mark"]["point"] == false
    end

    test "steps between thresholds instead of interpolating" do
      assert curve_layer(Verdict.roc_curve(y_true(), scores()))["mark"]["interpolate"] ==
               "step-after"
    end

    test "pins both axes to 0..1" do
      layer = curve_layer(Verdict.roc_curve(y_true(), scores()))
  ...
  describe "precision_recall_curve/3" do
    test "plots the values scikit-learn computes" do
      points = values(Verdict.precision_recall_curve(y_true(), scores()))
    ...
    test "puts average precision in the series label" do
      plot = Verdict.precision_recall_curve([{"Logistic", y_true(), scores()}])
      assert [%{"series" => label} | _] = values(plot)
      assert label =~ "AP = "
    end
    ...
    test "baselines at the share of positives, not the diagonal" do
      ...
      [reference, _curve] = spec(Verdict.precision_recall_curve(y_true(), scores()))["layer"]
  ...
  describe "det_curve/3" do
    test "plots false negative against false positive rate" do
      plot = Verdict.det_curve(y_true(), scores())
    ...
    test "has no chance reference" do
      assert length(spec(Verdict.det_curve(y_true(), scores()))["layer"]) == 1
    end

    test "leaves the axes unpinned, since rates here are not bounded to a corner" do
      layer = curve_layer(Verdict.det_curve(y_true(), scores()))
