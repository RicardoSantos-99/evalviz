defmodule Verdict.SampleWeightsTest do
  ...
  describe "weighting equals duplicating" do
    test "confusion matrix" do
      assert_same(
        Verdict.confusion_matrix(y_true(), y_pred(), num_classes: 2, sample_weights: weights()),
        Verdict.confusion_matrix(doubled_true(), doubled_pred(), num_classes: 2),
        "value"
      )
    end
    ...
    test "roc curve" do
      assert_same(
        Verdict.roc_curve(y_true(), scores(), sample_weights: weights()),
        Verdict.roc_curve(doubled_true(), doubled_scores()),
        "tpr"
      )
    end
    ...
    test "precision-recall curve" do
      assert_same(
        Verdict.precision_recall_curve(y_true(), scores(), sample_weights: weights()),
        Verdict.precision_recall_curve(doubled_true(), doubled_scores()),
        "precision"
      )
    end
    ...
    test "threshold curve" do
      assert_same(
        Verdict.threshold_curve(y_true(), scores(), sample_weights: weights()),
        Verdict.threshold_curve(doubled_true(), doubled_scores()),
        "value"
      )
    end

    test "calibration curve" do
      weighted = Verdict.calibration_curve(y_true(), scores(), bins: 2, sample_weights: weights())
      duplicated = Verdict.calibration_curve(doubled_true(), doubled_scores(), bins: 2)
    ...
    test "score distribution" do
      assert_same(
        Verdict.score_distribution(y_true(), scores(), bins: 3, sample_weights: weights()),
        Verdict.score_distribution(doubled_true(), doubled_scores(), bins: 3),
        "value"
      )
    end
  end
  ...
  describe "what the weights change" do
    ...
    test "the Brier score is weighted too" do
      weighted =
        Verdict.calibration_curve([{"m", y_true(), scores()}], bins: 2, sample_weights: weights())

      duplicated = Verdict.calibration_curve([{"m", doubled_true(), doubled_scores()}], bins: 2)
  ...
  describe "rejections" do
    test "weights that do not line up with the samples" do
      assert_raise ArgumentError, ~r/one sample weight per sample/, fn ->
        Verdict.roc_curve(y_true(), scores(), sample_weights: [1, 2])
      end
    end

    test "weights that are not a flat list" do
      assert_raise ArgumentError, ~r/:sample_weights to be rank 1/, fn ->
        Verdict.roc_curve(y_true(), scores(), sample_weights: Nx.broadcast(1.0, {5, 2}))
      end
    end
    ...
    test "weights alongside the micro average" do
      ...
      assert_raise ArgumentError, ~r/cannot be combined with the micro average/, fn ->
        Verdict.roc_curve(multi, matrix, average: :micro, sample_weights: [1, 2, 1])
      end
    end
    ...
    test "but per-class curves take weights fine" do
      ...
      assert %VegaLite{} = Verdict.roc_curve(multi, matrix, sample_weights: [1, 2, 1])
    end
  end
end
