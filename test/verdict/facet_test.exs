defmodule Verdict.FacetTest do
  ...
  describe "faceting the one-vs-rest curves" do
    test "splits into a facet spec rather than one set of axes" do
      plot = Verdict.roc_curve(y_true(), scores(), facet: true)
    ...
    test "wraps the panels at the column count" do
      assert spec(Verdict.roc_curve(y_true(), scores(), facet: true))["columns"] == 3
      assert spec(Verdict.roc_curve(y_true(), scores(), facet: true, columns: 2))["columns"] == 2
    end

    # Otherwise the header prints the field's own name above the panels.
    test "leaves the facet header unlabelled" do
      assert Map.fetch!(
               spec(Verdict.roc_curve(y_true(), scores(), facet: true))["facet"],
               "title"
             ) ==
               nil
    end
    ...
    test "carries the title over the whole thing" do
      plot = Verdict.roc_curve(y_true(), scores(), facet: true, title: "Six ways")
      assert spec(plot)["title"] == "Six ways"
    end

    test "has nothing to split with a single curve" do
      plot =
        Verdict.roc_curve(Nx.tensor([0, 0, 1, 1]), Nx.tensor([0.1, 0.4, 0.35, 0.8]), facet: true)
  ...
  describe "the precision-recall baseline under faceting" do
    ...
    test "is dropped when overlaid but drawn per panel when faceted" do
      overlaid = Verdict.precision_recall_curve(y_true(), scores())
      faceted = Verdict.precision_recall_curve(y_true(), scores(), facet: true)
    ...
    test "gives each class its own share of positives" do
      ...
      baselines =
        Verdict.precision_recall_curve(y_true(), scores(), facet: true)
        |> values()
    ...
    test "gives the macro curve the average of the per-class ones" do
      rows = values(Verdict.precision_recall_curve(y_true(), scores(), average: :macro))
    ...
    test "keeps every row agreeing on its columns" do
      rows = values(Verdict.precision_recall_curve(y_true(), scores(), average: [:micro, :macro]))
  ...
  describe "faceting calibration" do
    test "splits into a panel per class" do
      plot = Verdict.calibration_curve(y_true(), scores(), bins: 2, facet: true)
  ...
  describe "faceting the threshold curve" do
    test "splits on the class rather than the curve" do
      plot = Verdict.threshold_curve(y_true(), scores(), facet: true)
