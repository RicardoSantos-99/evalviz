defmodule Verdict.DendrogramTest do
  ...
  describe "dendrogram/2" do
    test "returns a VegaLite spec" do
      assert %VegaLite{} = Verdict.dendrogram(model())
    end

    test "accepts a {clades, heights} pair as well as a model" do
      m = model()
      from_model = values(Verdict.dendrogram(m))
      from_pair = values(Verdict.dendrogram({m.clades, m.dissimilarities}))

      assert from_model == from_pair
    end

    test "orders leaves the way scipy does" do
      label_expr = spec(Verdict.dendrogram(model()))["encoding"]["x"]["axis"]["labelExpr"]
    ...
    test "draws the same brackets scipy does" do
      ...
      assert brackets(Verdict.dendrogram(model())) == [
    ...
    test "color_threshold splits the tree into the clusters below the cut" do
      plot = Verdict.dendrogram(model(), color_threshold: 1.0)
    ...
    test "a threshold above every merge leaves one cluster and nothing above it" do
      plot = Verdict.dendrogram(model(), color_threshold: 10.0)
