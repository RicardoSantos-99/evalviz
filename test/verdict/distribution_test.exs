defmodule Verdict.DistributionTest do
  ...
  describe "residual_distribution/3" do
    ...
    test "returns a VegaLite spec" do
      assert %VegaLite{} = Verdict.residual_distribution(y_true(), y_pred())
    end

    test "counts every residual exactly once" do
      counts = Verdict.residual_distribution(y_true(), y_pred(), bins: 4) |> values()
    ...
    test "takes the residual as prediction minus truth, like residuals/3" do
      ...
      rows = Verdict.residual_distribution(y_true(), y_pred(), bins: 3) |> values()
    ...
    test "puts the largest value in the last bin rather than past it" do
      ...
      assert %VegaLite{} = Verdict.qq_plot(values)
    end

    test "draws the zero a residual with no bias would centre on" do
      assert length(spec(Verdict.residual_distribution(y_true(), y_pred()))["layer"]) == 2
    end

    test "zero_line: false leaves it out" do
      plot = Verdict.residual_distribution(y_true(), y_pred(), zero_line: false)
      assert length(spec(plot)["layer"]) == 1
    end

    test "rejects inputs of different lengths" do
      assert_raise ArgumentError, ~r/same length/, fn ->
        Verdict.residual_distribution(y_true(), Nx.tensor([1.0, 2.0]))
      end
    end
  end
  ...
  describe "qq_plot/2" do
    ...
    test "returns a VegaLite spec" do
      assert %VegaLite{} = Verdict.qq_plot(sample())
    end

    test "sorts the sample up the y axis" do
      assert Verdict.qq_plot(sample()) |> values() |> Enum.map(& &1["sample"]) ==
    ...
    test "fits the reference line the way scipy does" do
      ...
      [p1, p2] = spec(Verdict.qq_plot(sample()))["layer"] |> hd() |> get_in(["data", "values"])
    ...
    test "reference_line: false leaves the fit out" do
      assert length(spec(Verdict.qq_plot(sample(), reference_line: false))["layer"]) == 1
    end

    test "needs at least two values" do
      assert_raise ArgumentError, ~r/at least two values/, fn ->
        Verdict.qq_plot(Nx.tensor([1.0]))
      end
    end
  end
end
